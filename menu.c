...
static void control_change() {
    ...
    if (mouseplayer < 0)
        mouseplayer = -1;
    if (mouseplayer >= nrockets)
        mouseplayer = -1;
#endif
#ifdef JOYSTICK
    if (joystickplayer[0] < 0)
        joystickplayer[0] = -1;
    if (joystickplayer[0] >= nrockets)
        joystickplayer[0] = -1;
    if (joystickplayer[1] < 0)
        joystickplayer[1] = -1;
    if (joystickplayer[1] >= nrockets)
        joystickplayer[1] = -1;
#ifdef NETSUPPORT
    if (client && !control[joystickplayer[0]])
        joystickplayer[0] = -1;
    if (client && !control[joystickplayer[1]])
        joystickplayer[1] = -1;
    if (client && !control[player])
        return;
#endif
    ...
    maybejoystick:;
        ...
        if (joystickmul[0] == JOYMUL1)
            joystickmul[0] = JOYMUL2;
        else {
            joystickplayer[0] = -1;
        ...
        if (joystickmul[1] == JOYMUL1)
            joystickmul[1] = JOYMUL2;
        else
            joystickplayer[1] = -1;
#endif
...
static void change_menu() {
    ...
    for (i = 0; i < nrockets; i++) {
        ...
        if (control[i] == 0 && client)
            ...
            if (joystickmul[0] == JOYMUL1)
                s1 = "JOYSTICK A button";
            else
                s1 = "JOYSTICK A deflection";
        } else if (i == joystickplayer[1]) {
            if (joystickmul[1] == JOYMUL1)
                s1 = "JOYSTICK B button";
            else
                s1 = "JOYSTICK B deflection";
        } else
#endif
            if (rotation[i])
...
void cmenu2(unsigned char *message, int size) {
    printf("entering change menu");
    change_menu();
    gamemode = MENU;
}

static void cmain_menu() {
    nnumbers = 1;
    nmenu = 4;
...
void draw_menu(CONST int draw) {
    ...
    if (draw) {
        ...
        //      DrawBlackMaskedText (physicalscreen, MAPWIDTH / 2 - 33 * 4 + 1, 46, "PRESS L FOR HELP / START TO
        //      PAUSE"); DrawBlackMaskedText (physicalscreen, MAPWIDTH / 2 - 20 * 4 + 1, 46, "PRESS START TO PAUSE");
...
void menu_keys() {
    ...
    if (but) {
        int i;
        for (i = 0; i < nnumbers; i++) {
            if (MouseX() > maxim[i].x && MouseX() < maxim[i].x + 8 && MouseY() > maxim[i].y &&
                MouseY() < maxim[i].y + 8)
                increase(i);
            if (MouseX() > minim[i].x && MouseX() < minim[i].x + 8 && MouseY() > minim[i].y &&
                MouseY() < minim[i].y + 8)
                decrease(i);
        }
    }
