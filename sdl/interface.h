...
extern Uint32
    colors_RGBA[256]; // Stores full RGBA representation of palette for both 32bpp display and gfx primitives use
#ifndef SDL32BPP
...
extern int
    gsensor_recently_recentered; // This signals code in koules.c to display message confirming recentering of g-sensor
#endif
...
#define SSetPixel(passed_surface, x, y, color) \
    (((Uint32 *) (passed_surface->pixels))[((y) *passed_surface->w) + (x)] = (Uint32)(colors_RGBA[(color)]))
#else
#define SGetPixel(passed_surface, x, y) (((Uint16 *) (passed_surface->pixels))[((y) *passed_surface->w) + (x)])
#define SSetPixel(passed_surface, x, y, color) \
    (((Uint16 *) (passed_surface->pixels))[((y) *passed_surface->w) + (x)] = (Uint16)(colors_16bit[(color)]))
#endif
...
#define DrawBlackMaskedText(passed_surface, x, y, text) SFont_Write((passed_surface), fontblack, (x), (y), (text))

#define DrawWhiteMaskedText(passed_surface, x, y, text) SFont_Write((passed_surface), fontwhite, (x), (y), (text))

#define DrawGrayMaskedText(passed_surface, x, y, text) SFont_Write((passed_surface), fontgray, (x), (y), (text))
...
//}
//#define DrawRectangle(passed_surface, x1, y1, x2, y2, color) rectangleColor((passed_surface), (x1), (y1), (x2), (y2),
//colors_RGBA[(color)]) DKS - actually, now it's a bit different
#define DrawRectangle(passed_surface, x1, y1, x2, y2, color) sdl_rectangle((x1), (y1), (x2), (y2), (color))
...
//#define FillRectangle(passed_surface,x,y,x1,y1,color) boxColor(passed_surface, (x), (y), (x1), (y1),
//colors_RGBA[(color)])
...
#define Line(passed_surface, x1, y1, x2, y2, color) \
    SDL_DrawLine((passed_surface), (x1), (y1), (x2), (y2), colors_RGBA[(color)])
