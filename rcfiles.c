...
//  sprintf (fullname, "%s", levelsname);
    ...
    // DKS - holy cow, fwrite is being used here with parameters reversed, gotta fix that (how did this ever work
    // originally!?)
...
//  sprintf (fullname, "%s", levelsname);
    ...
    // DANGER DANGER DANGER
    // DKS - holy cow, fread is being used here with parameters reversed, gotta fix that (how did this ever work
    // originally!?) DKS - Not only that, but it misinterprets the return result of fread()!  Unbelievable..
