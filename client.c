...
#define WAITTIME 5
void csendbegining(unsigned char *message, int size);

void csetrrcount(long scount) {
...
void csendbuffer() {
    ...
    if ((result = DgramSend(sock, servername, port, (char *) obuffer, obufferpos + 4)) <= 0) {
...
void csendbegining(unsigned char *message, int size) /*send at more reliable beggining of packet */
{
...
void csendreliable(unsigned char *message, int size) {
    if (size == 0)
        return;
...
static void print(unsigned char *message, int size) {
    printf("Server's message:%s\n", (char *) message);
}
static void quit(unsigned char *message, int size) {
    printf("Uninitializing:%s\n", (char *) message);
    SocketClose(sock);
    quited = 1;
    exit(0);
}
static void cexplosion(unsigned char *message, int size) {
...
static void coutro(unsigned char *message, int size) {
    outro2();
    CQuit("End of game\nBye");
}
static void ccreatord_points(unsigned char *message, int size) {
...
static void csound(unsigned char *message, int size) {
    int s;
    GETCHAR(message, s);
    Effect(s, next);
}

static void cplayers(unsigned char *message, int size) {
    int s;
    GETCHAR(message, s);
    setnplayers(s);
}
static void creg(unsigned char *message, int size) {
    int s;
...
static void cpos(unsigned char *message, int size) {
    read_objects((unsigned char *) message, nodata);
    rframe = 1;
    nodata = 0;
}
static void clevel(unsigned char *message, int size) {
    int nos;
...
static void cgame(unsigned char *message, int size) {
    gamemode = GAME;
    Ready();
}
void creliable(unsigned char *message, int size);
static struct c_table ctable[] = {{print}, {quit},   {cpos},   {cexplosion}, {ccreatord_points}, {csound}, {cplayers},
                                  {creg},  {cmenu2}, {clevel}, {cgame},      {creliable},        {coutro}};
void creliable(unsigned char *message, int size) {
    int pos = 0;
    while (pos < size) {
        long rpos1;
...
again:
    ...
    do {
        ...
        if (DgramSend(socket_c, servername, initport, (char *) buffer, INITPACKETSIZE) == -1) {
            SocketClose(socket_c);
            perror("Can't send message to server");
            exit(-1);
        }
    ...
    if (gx / DIV > GAMEWIDTH || gy / DIV > GAMEHEIGHT) {
        ...
        PUTLONG(buffer, 0);
        if (DgramSend(sock, servername, port, (char *) buffer, 4) == -1)
            close(sock);
        exit(-1);
    } else
...
void client_loop(void) {
  ...
  int             display = 1;
    ...
    while (1) {
        ...
        SendControls();
#ifndef HAVEUSLEEP
        do { /*my emulation of usleep isn't reiable on HP-UX machines
