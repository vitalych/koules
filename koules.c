...
//                ehole_bitmap;
SDL_Surface *bball_bitmap = NULL, *apple_bitmap = NULL, *inspector_bitmap = NULL, *mouse_bitmap = NULL,
            *lunatic_bitmap = NULL, *lball_bitmap[NLETTERS], *circle_bitmap = NULL, *hole_bitmap = NULL,
            *ball_bitmap = NULL, *eye_bitmap[MAXROCKETS], *rocket_bitmap[MAXROCKETS], *ehole_bitmap = NULL;

// DKS - original, had to change for platforms not supporting unaligned access
// unsigned char   rocketcolor[5] =
//{96, 160, 64, 96, 128};
unsigned int rocketcolor[5] = {96, 160, 64, 96, 128};
...
int a_bballs, a_rockets, a_balls, a_holes, a_apples, a_inspectors, a_lunatics, a_eholes;
...
#ifdef JOYSTICK
int joystickplayer[2] = {-1, -1};
int joystickdevice[2] = {-1, -1};
int calibrated[2];
int center[2][2];
...
int find_possition(float *x, float *y, CONST float radius) {
    int x1, y1, i, y2 = 0;
    float xp, yp;
rerand:;
    ...
    for (i = 0; i < nobjects; i++) {
        xp = x1 - object[i].x;
        yp = y1 - object[i].y;
        if (xp * xp + yp * yp < (radius + object[i].radius) * (radius + object[i].radius)) {
            y2++;
            if (y2 > 10000)
                return (0);
            goto rerand;
...
INLINE void normalize(float *x, float *y, CONST float size) {
...
static void draw_objects(CONST int draw) {
    ...
    if (draw) {
        ...
        for (i = 0; i < nobjects; i++)
            if (object[i].live && object[i].lineto != -1 && object[object[i].lineto].live) {
                ...
                Line(physicalscreen, object[i].x / DIV, object[i].y / DIV, object[object[i].lineto].x / DIV,
                     object[object[i].lineto].y / DIV, 255);

                help((object[i].x + object[object[i].lineto].x) / 2, (object[i].y + object[object[i].lineto].y) / 2, 2,
                     "Spit");
            }
        for (i = 0; i < nobjects; i++)
            if (object[i].live) {
                ...
                switch (object[i].type) {
                    case BALL:
                        ...
                        // DKS
                        //		PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y -
                        //BALL_RADIUS) / DIV, 		 BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, ball_bitmap);
                    ...
                    case LBALL:
                        switch (object[i].letter) {
                            case L_ACCEL:
                                // DKS
                                //		    PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y
                                //- BALL_RADIUS) / DIV, 			       BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, lball_bitmap[0]);
                                PutBitmap((int) (object[i].x - BALL_RADIUS) / DIV,
                                          (int) (object[i].y - BALL_RADIUS) / DIV, lball_bitmap[0], physicalscreen);
                            ...
                            case L_GUMM:
                                ////DKS
                                //		    PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y
                                //- BALL_RADIUS) / DIV, 			       BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, lball_bitmap[1]);
                                PutBitmap((int) (object[i].x - BALL_RADIUS) / DIV,
                                          (int) (object[i].y - BALL_RADIUS) / DIV, lball_bitmap[1], physicalscreen);
                            ...
                            case L_THIEF:
                                // DKS
                                //		    PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y
                                //- BALL_RADIUS) / DIV, 			       BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, lball_bitmap[2]);
                                PutBitmap((int) (object[i].x - BALL_RADIUS) / DIV,
                                          (int) (object[i].y - BALL_RADIUS) / DIV, lball_bitmap[2], physicalscreen);
                            ...
                            case L_FINDER:
                                // DKS
                                //		    PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y
                                //- BALL_RADIUS) / DIV, 			       BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, lball_bitmap[3]);
                                PutBitmap((int) (object[i].x - BALL_RADIUS) / DIV,
                                          (int) (object[i].y - BALL_RADIUS) / DIV, lball_bitmap[3], physicalscreen);
                            ...
                            case L_TTOOL:
                                // DKS
                                //		    PutBitmap ((int) (object[i].x - BALL_RADIUS) / DIV, (int) (object[i].y
                                //- BALL_RADIUS) / DIV, 			       BALL_RADIUS * 2 / DIV, BALL_RADIUS * 2 / DIV, lball_bitmap[4]);
                                PutBitmap((int) (object[i].x - BALL_RADIUS) / DIV,
                                          (int) (object[i].y - BALL_RADIUS) / DIV, lball_bitmap[4], physicalscreen);

                                help(object[i].x, object[i].y, object[i].radius, "Thief toolkit");
                                break;
                        }

                        // DKS - This is for when running at higher resolutions, where a letter is placed over the
                        // balls..
                        // we don't need this..
                        //
                        ...
                        //		    DrawBlackMaskedText ((int) object[i].x / DIV - 4, (int) object[i].y / DIV - 4,
                        //str);
                        //		  }
                        //#endif
                    ...
                    case HOLE:
                        ...
                        // DKS
                        //		PutBitmap ((int) (object[i].x - HOLE_RADIUS) / DIV, (int) (object[i].y -
                        //HOLE_RADIUS) / DIV, 			   HOLE_RADIUS * 2, HOLE_RADIUS * 2, hole_bitmap);
                    ...
                    case EHOLE:
                        ...
                        // DKS
                        //		PutBitmap ((int) (object[i].x - HOLE_RADIUS) / DIV, (int) (object[i].y -
                        //HOLE_RADIUS) / DIV, 			   HOLE_RADIUS * 2, HOLE_RADIUS * 2, ehole_bitmap);
                        PutBitmap((int) (object[i].x - HOLE_RADIUS) / DIV, (int) (object[i].y - HOLE_RADIUS) / DIV,
                    ...
                    case BBALL:
                        // DKS
                        //		PutBitmap ((int) (object[i].x - BBALL_RADIUS) / DIV, (int) (object[i].y -
                        //BBALL_RADIUS) / DIV, 			   BBALL_RADIUS * 2 / DIV, BBALL_RADIUS * 2 / DIV, bball_bitmap);
                    ...
                    case INSPECTOR:
                        // DKS
                        //		PutBitmap ((int) (object[i].x - INSPECTOR_RADIUS) / DIV, (int) (object[i].y -
                        //INSPECTOR_RADIUS) / DIV, 			   INSPECTOR_RADIUS * 2 / DIV, INSPECTOR_RADIUS * 2 / DIV,
                        //inspector_bitmap);
                        PutBitmap((int) (object[i].x - INSPECTOR_RADIUS) / DIV,
                                  (int) (object[i].y - INSPECTOR_RADIUS) / DIV, inspector_bitmap, physicalscreen);

                        help(object[i].x, object[i].y, object[i].radius, "Inspector");
                        break;
                    case LUNATIC:
                        // DKS
                        //		PutBitmap ((int) (object[i].x - LUNATIC_RADIUS) / DIV, (int) (object[i].y -
                        //LUNATIC_RADIUS) / DIV, 			   LUNATIC_RADIUS * 2 / DIV, LUNATIC_RADIUS * 2 / DIV, lunatic_bitmap);
                        PutBitmap((int) (object[i].x - LUNATIC_RADIUS) / DIV,
                                  (int) (object[i].y - LUNATIC_RADIUS) / DIV, lunatic_bitmap, physicalscreen);

                        help(object[i].x, object[i].y, object[i].radius, "Lunatic");
                        break;
                    case APPLE:
                        // DKS
                        //		PutBitmap ((int) (object[i].x - APPLE_RADIUS) / DIV, (int) (object[i].y -
                        //APPLE_RADIUS) / DIV, 			   APPLE_RADIUS * 2 / DIV, APPLE_RADIUS * 2 / DIV, apple_bitmap);
                        ...
                        // DKS
                        //		Line ((int) (object[i].x + 10) / DIV, (int) (object[i].y - APPLE_RADIUS - 10) /
                        //DIV, 		      (int) (object[i].x) / DIV, (int) (object[i].y - APPLE_RADIUS + 10) / DIV, 150); 		Line
                        //((int) (object[i].x + 10) / DIV + 1, (int) (object[i].y - APPLE_RADIUS - 10) / DIV, 		      (int)
                        //(object[i].x) / DIV + 1, (int) (object[i].y - APPLE_RADIUS + 10) / DIV, 150);

                        Line(physicalscreen, (int) (object[i].x + 10) / DIV,
                             (int) (object[i].y - APPLE_RADIUS - 10) / DIV, (int) (object[i].x) / DIV,
                             (int) (object[i].y - APPLE_RADIUS + 10) / DIV, 150);
                        Line(physicalscreen, (int) (object[i].x + 10) / DIV + 1,
                             (int) (object[i].y - APPLE_RADIUS - 10) / DIV, (int) (object[i].x) / DIV + 1,
                             (int) (object[i].y - APPLE_RADIUS + 10) / DIV, 150);

                        // DKS no DIV of 1 on GP2X
                        //		if (DIV == 1)
                        //		  Line ((int) (object[i].x + 10) / DIV + 2, (int) (object[i].y - APPLE_RADIUS -
                        //10) / DIV, 			(int) (object[i].x) / DIV + 2, (int) (object[i].y - APPLE_RADIUS + 10) / DIV, 150);
                        ...
                        PutBitmap((int) (object[i].x - EYE_RADIUS) / DIV, (int) (object[i].y + APPLE_RADIUS - 15) / DIV,
                    ...
                    case ROCKET: {
                        ...
                        // DKS
                        //		  PutBitmap ((int) (object[i].x - ROCKET_RADIUS) / DIV, (int) (object[i].y -
                        //ROCKET_RADIUS) / DIV, 			     ROCKET_RADIUS * 2 / DIV, ROCKET_RADIUS * 2 / DIV, rocket_bitmap[i]);
                        ...
                        if (!object[i].thief) {
                            ...
                            //				 (int) (EYE_RADIUS * 2 / DIV), (EYE_RADIUS * 2 / DIV),
                            //eye_bitmap[i]);
                            PutBitmap((int) (x1 / DIV), (int) (y1 / DIV), eye_bitmap[i], physicalscreen);
                            ...
                            // DKS
                            //		      PutBitmap ((int) (x1 / DIV), (int) (y1 / DIV),
                            //				 (int) (BALL_RADIUS * 2 / DIV), (BALL_RADIUS * 2 / DIV),
                            //lball_bitmap[2]);
                            PutBitmap((int) (x1 / DIV), (int) (y1 / DIV), lball_bitmap[2], physicalscreen);
    ...
    if (draw) {
        ...
        if (in_actual_game) {
            ...
            //	       object[4].score);
            if (nrockets > 1)
                sprintf(s, "   Score: %6i", object[0].score);
            else
                sprintf(s, "Score: %6i", object[0].score);
...
void explosion(CONST int x, CONST int y, CONST int type, CONST int letter, CONST int n) {
    ...
    for (i = 0; i < RAD(360); i += RAD(360.0) * DIV * DIV / radius1 / radius1 / M_PI) {
        ...
        addpoint(x * 256, y * 256, sin(i) * (speed), cos(i) * (speed), color1, rand() % 100 + 10);
    }
}
    ...
    switch (object[i].type) {
        ...
        case ROCKET:
            ...
            object[i].live1--, object[i].live--,
                explosion(object[i].x, object[i].y, object[i].type, object[i].letter, i);
            rocket_destroyed(i);
            if (object[i].live) {
...
#endif
    {
        ...
        if (!server)
            ...
            for (y = 0; y < 5 / DIV / DIV; y++) {
                float p;
                p = RAD(rand() % 45 - 22);
                addpoint(object[i].x * 256, object[i].y * 256,
        ...
        for (y = 0; y < 5 / DIV / DIV; y++) {
            ...
            addpoint(object[i].x * 256, object[i].y * 256,
                     ...
                     (-cos(object[i].rotation + p) * ROCKET_SPEED * 5) * (rand() % 512), rocket(rand() % 16), 10);
        }
    }
#endif
}
        ...
        if (object[i].live && object[i].type == ROCKET) {
            switch (controls[i].type) {
                ...
                case C_JOYSTICK1: {
                    ...
                    if ((a > 1.0) || (controls[i].mask != 0))
                        a = 1.0;
                    if (a > object[i].joythresh)
                        accel(i, a);
                } break;
#endif
                case C_MOUSE: {
        ...
        case GAME:
            ...
            for (i = 0; i < nrockets; i++) {
...
#endif
#ifdef JOYSTICK
                if (i == joystickplayer[0] || i == joystickplayer[1])
                    continue;
#endif
#ifdef NETSUPPORT
                if (client && !control[i])
                ...
                if (rotation[i]) {
                    ...
                    int s = 0;
                    if (nrockets == 1) { // DKS - only playing one player mode:
                        // .. that means player can control using either DPAD, A/B/X/Y (as dpad), analog stick, or
                        // gsensor
                        if ((IsPressedUp() && IsPressedLeft()) || (IsPressedAltUp() && IsPressedAltLeft()) ||
                            (IsPressedAnalogUp() && IsPressedAnalogLeft()) ||
                            (IsPressedGsensorUp() && IsPressedGsensorLeft()))
                            s = 1;
                        else if ((IsPressedUp() && IsPressedRight()) || (IsPressedAltUp() && IsPressedAltRight()) ||
                                 (IsPressedAnalogUp() && IsPressedAnalogRight()) ||
                                 (IsPressedGsensorUp() && IsPressedGsensorRight()))
                            s = 2;
                        else if ((IsPressedDown() && IsPressedRight()) || (IsPressedAltDown() && IsPressedAltRight()) ||
                                 (IsPressedAnalogDown() && IsPressedAnalogRight()) ||
                                 (IsPressedGsensorDown() && IsPressedGsensorRight()))
                            s = 3;
                        else if ((IsPressedDown() && IsPressedLeft()) || (IsPressedAltDown() && IsPressedAltLeft()) ||
                                 (IsPressedAnalogDown() && IsPressedAnalogLeft()) ||
                                 (IsPressedGsensorDown() && IsPressedGsensorLeft()))
                            s = 4;
                        else if (IsPressedLeft() || IsPressedAltLeft() || IsPressedAnalogLeft() ||
                                 IsPressedGsensorLeft())
                            s = 5;
                        else if (IsPressedRight() || IsPressedAltRight() || IsPressedAnalogRight() ||
                                 IsPressedGsensorRight())
                            s = 6;
                        else if (IsPressedUp() || IsPressedAltUp() || IsPressedAnalogUp() || IsPressedGsensorUp())
                            s = 7;
                        else if (IsPressedDown() || IsPressedAltDown() || IsPressedAnalogDown() ||
                                 IsPressedGsensorDown())
                            s = 8;
                    } else {
                        if (i == 0) {
...
void creators_points(int radius, int x1, int y1, int color1) {
    ...
    while (z--) {
        ...
        addpoint(x * 256, y * 256, (x2 - x) * 256 / (time), (y2 - y) * 256 / (time),
                 color1 + (rand() % (DIV == 1 ? 16 : 32)), time);
    }
}
...
void creator(CONST int type) {
    ...
    for (i = nrockets; i < nobjects && (object[i].live || object[i].type == CREATOR); i++)
        ;
    if (i >= MAXOBJECT)
        return;
    ...
    for (i = 0; i < nobjects; i++) {
        if (object[i].live) {
            ...
            if (object[i].type == BALL || object[i].type == LBALL || object[i].type == BBALL ||
                ...
                if (frocket != -1)
                    xp = object[frocket].x - object[i].x, yp = object[frocket].y - object[i].y;
                else
                    xp = GAMEWIDTH / 2 - object[i].x, yp = GAMEHEIGHT / 2 - object[i].y;
            ...
            object[i].fx *= SLOWDOWN, object[i].fy *= SLOWDOWN;
        }
    }
}
        ...
        if (object[i].live)
            for (y = i + 1; y < nobjects; y++)
                if (object[y].live) {
                    ...
                    if (xp * xp + yp * yp <
                        (object[y].radius + object[i].radius) * (object[y].radius + object[i].radius)) {
                        colize = 1;
                        ...
                        if (object[i].type == ROCKET) {
                            ...
                            if (object[y].letter == L_ACCEL)
                                object[i].accel += A_ADD, object[i].score += 10;
                            if (object[y].letter == L_GUMM)
                                object[i].M += M_ADD, object[i].score += 10;
                            if (object[y].letter == L_THIEF)
                                object[i].M = M(object[i].type), object[i].accel = ROCKET_SPEED - A_ADD,
                                object[i].score -= 30;
                            ...
                            if (gameplan == DEATHMATCH && object[y].type == ROCKET && dosprings &&
                                !(rand() % (2 * randsprings)))
                                object[y].lineto = i;
                        }
...
void game() {
    ...
    while (1) {
        ...
        gettimeofday(&VlastClk, NULL);
        if (tbreak)
            tbreak = 0, VendSleep = VlastClk.tv_usec;
    }
}
#ifdef NETSUPPORT
void client_loop2(int draw) /*game part of server loop */
{
    draw_objects(draw);
    switch (gamemode) {
        case MENU:
...
void server_loop2(void) /*game part of server loop */
{
    sprocess_keys();
    update_values();
    update_game();
