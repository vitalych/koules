...
MRESULT EXPENTRY vgaAppWindowProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
        ...
        switch (msg) {
            ...
            case WM_MOUSEMOVE:
                ...
                if (!vga_os2_FSDiveFullScreen()) {
                    ...
                    mousey = (MAPHEIGHT + 19) - ((MOUSEMSG(&msg)->y) * _pwinData->ulHeight) / (swp.cy);
                    ...
                    mousey = ((RealScreenY - 1 - MOUSEMSG(&msg)->y) * _pwinData->ulHeight) / 25;
                }
                ...
                switch (SHORT1FROMMP(mp1)) {
                    case IDM_DESKSCR:
                        if (vga_os2_presentFSDive() && vga_os2_FSDiveFullScreen())
                            WinSendMsg(_pwinData->hwndFrame, WM_SetVideoMode, (MPARAM) WS_DesktopDive, 0);
                        break;

                    case IDM_FULLSCR: /* GameSrvr */
                        if (vga_os2_presentFSDive())
                            WinSendMsg(_pwinData->hwndFrame, WM_SetVideoMode, (MPARAM) WS_FullScreenDive, 0);
                        break;
                    ...
                    case IDM_SNAP: {
                        ...
                        ulHeight += WinQuerySysValue(HWND_DESKTOP, SV_CYSIZEBORDER) * 2;
                        ulHeight += WinQuerySysValue(HWND_DESKTOP, SV_CYBORDER) * 2;
                        ulHeight += WinQuerySysValue(HWND_DESKTOP, SV_CYMENU);
                        ulHeight += WinQuerySysValue(HWND_DESKTOP, SV_CYTITLEBAR);
                        ulWidth += WinQuerySysValue(HWND_DESKTOP, SV_CXSIZEBORDER) * 2;

                        /* Set the new size of the window, but don't move
                         * the lower-left corner.
                         */
                        WinSetWindowPos(_pwinData->hwndFrame, NULLHANDLE, 100, 100, ulWidth, ulHeight,
                    ...
                    case IDM_CENTER: /* Center windows */
                    {
                        ...
                        WinSetWindowPos(_pwinData->hwndFrame, HWND_TOP, WinPosX, WinPosY, 0, 0,
            ...
            case WM_CHAR:
                ...
                /* Check for KeyPress */
                    ...
                    if (((CHARMSG(&msg)->fs & KC_KEYUP) == 0) && ((CHARMSG(&msg)->fs & KC_PREVDOWN) == 0)) {
