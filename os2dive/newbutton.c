...
BOOL APIENTRY RegisterNewBtnClass(HAB hab) {
    return WinRegisterClass(hab, "NewBtn", NewBtnWndProc, CS_SIZEREDRAW, sizeof(PNEWBTN));
}
    ...
    switch (msg) {
        ...
        case WM_BUTTON1UP:
            ...
            if (WinPtInRect(WinQueryAnchorBlock(hwnd), &rcl, &ptl))
                WinPostMsg(WinQueryWindow(hwnd, QW_OWNER), WM_COMMAND, MPFROMSHORT(WinQueryWindowUShort(hwnd, QWS_ID)),
        ...
        case WM_CHAR:
            if (!(CHARMSG(&msg)->fs & KC_VIRTUALKEY) || CHARMSG(&msg)->vkey != VK_SPACE ||
