...
static GraphicsContext *my_allocatecontext() {
...
int main(int argc, char *argv[]) {
    ...
    if (WinDlgBox(HWND_DESKTOP, HWND_DESKTOP, DlgProc, NULLHANDLE, IDD_DLGBOX, NULL)) {
        WinDestroyMsgQueue(hmq);
        WinTerminate(hab);
        exit(0);
    }
...
MRESULT EXPENTRY DlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
    ...
    switch (msg) {
        case WM_INITDLG:
            ...
            WinSetWindowPos(hwnd, HWND_DESKTOP, (USHORT)((WinQuerySysValue(HWND_DESKTOP, SV_CXSCREEN) - swp.cx) / 2),
                            (USHORT)((WinQuerySysValue(HWND_DESKTOP, SV_CYSCREEN) - swp.cy) / 2), 0, 0, SWP_MOVE);
        ...
        case WM_CONTROL:
            if (SHORT1FROMMP(mp1) >= IDD_320x200 && SHORT1FROMMP(mp1) <= IDD_640x480) {
                dlg_smode = SHORT1FROMMP(mp1);
            } else if (SHORT1FROMMP(mp1) >= IDD_COLORS && SHORT1FROMMP(mp1) <= IDD_BW) {
