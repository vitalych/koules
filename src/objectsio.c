...
/*bit counter */
static int c = 1;
unsigned char *write_object(unsigned char *buffer, int i) {
    unsigned long x;
    ...
    if (object[i].live && object[i].type == ROCKET) {
        while (object[i].rotation < 0)
            object[i].rotation += 2 * M_PI;
        while (object[i].rotation >= 2 * M_PI)
            object[i].rotation -= 2 * M_PI;
...
unsigned char *read_object(int i, unsigned char *buffer, int nodata) {
    unsigned long x;
    int a = 0;
...
int write_objects(unsigned char *buffer) {
    int i;
...
int read_objects(unsigned char *buffer, int nodata) {
    int i;
    unsigned char *buffer1 = buffer;
    c = 1;
    GETNUM(nobjects, 8);
