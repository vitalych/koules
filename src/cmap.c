...
// extern Uint16   colors_16bit[256]; //Stores 16-bit RGB representation of original palette for standard SDL pixel
// access on 16bpp machines
...
static INLINE int col(int p, const float p1) {
...
void setcustompalette(const int p, const float p1) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        colors_RGBA[i] = ((Uint32) col(r + p, p1) << 24) + ((Uint32) col(g + p, p1) << 16) +
                         ((Uint32) col(b + p, p1) << 8) + 0x000000FF;
#endif
    }
    ...
    for (i = 0; i < 32; i++) {
        ...
        colors_RGBA[i] = ((Uint32) col(i * 2 + p, p1) << 24) + ((Uint32) col(i * 2 + p, p1) << 16) +
                         ((Uint32) col(p, p1) << 8) + 0x000000FF;
#endif
    }
    for (i = 0; i < 32; i++) {
#ifdef SDL32BPP
        colors_RGBA[192 + i] =
            SDL_MapRGBA(physicalscreen->format, col(i * 2 + p, p1), col(i * 2 + p, p1), col(i * 2 + p, p1), 255);
#else
        colors_16bit[192 + i] =
            SDL_MapRGB(physicalscreen->format, col(i * 2 + p, p1), col(i * 2 + p, p1), col(i * 2 + p, p1));
        colors_RGBA[192 + i] = ((Uint32) col(i * 2 + p, p1) << 24) + ((Uint32) col(i * 2 + p, p1) << 16) +
                               ((Uint32) col(i * 2 + p, p1) << 8) + 0x000000FF;
#endif
    }
#ifdef SDL32BPP
    ...
    colors_RGBA[255] = ((Uint32) col(64 + p, p1) << 24) + ((Uint32) col(64 + p, p1) << 16) +
                       ((Uint32) col(64 + p, p1) << 8) + 0x000000FF;
#endif
}
...
void fade(SDL_Surface *p_surf_screen, SDL_Surface *p_surf_img, Uint32 ui_seconds, int b_fade_out) {
    ...
    if ((p_surf_black =
             SDL_CreateRGBSurface(ui_flags, p_surf_screen->w, p_surf_screen->h, p_surf_screen->format->BitsPerPixel,
