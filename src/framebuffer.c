...
static SDL_Surface *draw_ball_bitmap(int radius, const int color) {
    ...
    if ((newsurface = SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_RLEACCEL | SDL_SRCCOLORKEY, radius * 2, radius * 2,
                                           physicalscreen->format->BitsPerPixel, physicalscreen->format->Rmask,
                                           physicalscreen->format->Gmask, physicalscreen->format->Bmask,
                                           physicalscreen->format->Amask)) == NULL)
    ...
    for (y = 0; y < radius * 2; y++)
        for (x = 0; x < radius * 2; x++) {
            if ((x - radius) * (x - radius) + (y - radius) * (y - radius) < (radius - 0.5) * (radius - 0.5)) {
                r = (x - 3 * radius / 4) * (x - 3 * radius / 4) + (y - radius / 4) * (y - radius / 4);
                r = r * 32 / (1.5 * radius) / (1.5 * radius);
                if (r > 31)
                    r = 31;
                //*point = color + r;
...
// DKS - modified
static SDL_Surface *draw_reversed_ball_bitmap(int radius, CONST int color) {
    SDL_Surface *newsurface = NULL;
...
// DKS - modified
static SDL_Surface *draw_apple_bitmap(int radius, CONST int color) {
    SDL_Surface *newsurface = NULL;
    ...
    for (y = 0; y < radius * 2; y++)
        for (x = 0; x < radius * 2; x++) {
            ...
            if (radius1 > radius)
                radius1 = radius;
            if ((x - radius) * (x - radius) + (y - radius) * (y - radius) < ((radius1) * (radius1))) {
                r = (x - 3 * radius / 4) * (x - 3 * radius / 4) + (y - radius / 4) * (y - radius / 4);
                r = 3 + r * 22 / (1.5 * radius1) / (1.5 * radius1);
                if (r > 31)
                    r = 31;
...
//      {
//	if (DIV == 1)
//	  radius = HOLE_RADIUS / 2 + (int) (atan (fabs (x - HOLE_RADIUS + 0.5) / fabs (y - HOLE_RADIUS + 0.5)) *
//HOLE_RADIUS / 2) % (HOLE_RADIUS / 2); 	else 	  radius = HOLE_RADIUS / 4; 	if ((x - HOLE_RADIUS / DIV) * (x - HOLE_RADIUS /
//DIV) + (y - HOLE_RADIUS / DIV) * (y - HOLE_RADIUS / DIV) 	    < radius * radius)
//	  {
...
void create_bitmap() {
    ...
    if ((hole_bitmap = SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_RLEACCEL | SDL_SRCCOLORKEY, (HOLE_RADIUS * 2 / DIV),
                                            (HOLE_RADIUS * 2 / DIV), physicalscreen->format->BitsPerPixel,
                                            physicalscreen->format->Rmask, physicalscreen->format->Gmask,
                                            physicalscreen->format->Bmask, physicalscreen->format->Amask)) == NULL)
        printf("create_bitmap() framebuffer.c error"), exit(-1);
    if ((ehole_bitmap = SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_RLEACCEL | SDL_SRCCOLORKEY, (HOLE_RADIUS * 2 / DIV),
                                             (HOLE_RADIUS * 2 / DIV), physicalscreen->format->BitsPerPixel,
                                             physicalscreen->format->Rmask, physicalscreen->format->Gmask,
                                             physicalscreen->format->Bmask, physicalscreen->format->Amask)) == NULL)
        printf("create_bitmap() framebuffer.c error"), exit(-1);
        ...
        for (y = 0; y < HOLE_RADIUS * 2 / DIV; y++) {
            if (DIV == 1)
                radius = HOLE_RADIUS / 2 +
                         (int) (atan(fabs(x - HOLE_RADIUS + 0.5) / fabs(y - HOLE_RADIUS + 0.5)) * HOLE_RADIUS / 2) %
                             (HOLE_RADIUS / 2);
            else
    ...
    //	if (DIV == 1)
    //	  radius = HOLE_RADIUS / 2 + (int) (atan (fabs (x - HOLE_RADIUS + 0.5) / fabs (y - HOLE_RADIUS + 0.5)) *
    //HOLE_RADIUS / 2) % (HOLE_RADIUS / 2); 	else 	  radius = HOLE_RADIUS / 4; 	if ((x - HOLE_RADIUS / DIV) * (x -
    //HOLE_RADIUS / DIV) + (y - HOLE_RADIUS / DIV) * (y - HOLE_RADIUS / DIV) 	    < radius * radius)
    //	  {
    //	    r = (x - HOLE_RADIUS / DIV) * (x - HOLE_RADIUS / DIV) +
    //	      (y - HOLE_RADIUS / DIV) * (y - HOLE_RADIUS / DIV);
    //	    r = r * 24 / HOLE_SIZE_MAX;
...
#define Plasma_SSetPixel(passed_surface, x, y, color) \
    (((Uint32 *) (passed_surface->pixels))[((y) *passed_surface->w) + (x)] = (Uint32)(color))
...
static void createbackground() {
    ...
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++) {
            unsigned int color = (int) (+128.0 +
                                        (128.0 * cos(sqrt((x - circle_centerx) * (x - circle_centerx) +
                                                          (y - circle_centery) * (y - circle_centery)) /
                                                     circle_freq)) +
                                        128.0 +
                                        (128.0 * cos(sqrt((x - circle_centerx2) * (x - circle_centerx2) +
                                                          (y - circle_centery2) * (y - circle_centery2)) /
                                                     circle_freq2)) +
                                        128.0 +
                                        (128.0 * cos(sqrt((x - circle_centerx3) * (x - circle_centerx3) +
                                                          (y - circle_centery3) * (y - circle_centery3)) /
                                                     circle_freq3))) /
                                 3;
            color &= 0xFF;
