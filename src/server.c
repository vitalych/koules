...
static char str[30];
static char *s;
void edisplay(int e, int l, int t) { /*Magic routine */
...
void sinit(int client, unsigned char *message, int size) {
...
void sprint(int client, unsigned char *message, int size) {
    printf("Client's #%i message:%s\n", client, message);
}
void squit(int client, unsigned char *message, int size) {
    char str[400];
...
void spos(int client, unsigned char *message, int s) {
...
void sjoystick(int client, unsigned char *message, int s) {
    int p, x;
    GETCHAR(message + 5, p);
...
void smouse(int client, unsigned char *message, int s) {
    int p;
    GETCHAR(message + 5, p);
...
void skeys(int client, unsigned char *message, int s) {
    int p;
    GETCHAR(message + 1, p);
    controls[p].type = C_KEYBOARD;
    GETCHAR(message, controls[p].mask);
}
void srot(int client, unsigned char *message, int s) {
    int p;
    GETCHAR(message + 1, p);
    controls[p].type = C_RKEYBOARD;
    GETCHAR(message, controls[p].mask);
}
void sready(int client, unsigned char *message, int s) {
    conn[client].ready = 1;
    /*conn[client].wpos = 1; */
...
void sreg(int client, unsigned char *message, int s) {
    int n, i;
...
void SEffect(int level, int nos) {
    int i;
...
void sstart(int client, unsigned char *message, int s) {
    static int registered = 0;
    registered++;
...
struct s_table stable[] = {
    {sprint}, {squit}, {spos}, {sreg}, {sstart}, {sready}, {sjoystick}, {smouse}, {srot}, {skeys}, {sreliable}, {sinit},
};
...
void sflushreliable(int i) {
    if (conn[i].rpos) {
        PUTHEAD(CRELIABLE) memcpy(buffer + HEADSIZE, conn[i].rbuffer, conn[i].rpos);
        ssendbegining(i, buffer, conn[i].rpos + HEADSIZE);
    }
}
void ssendbuffer(int client) {
    int result;
    GetSocketError(conn[client].socket);
    if (conn[client].itimeout) {
        conn[client].itimeout--;
        if (!conn[client].itimeout) {
            if ((result = DgramSend(Socket, conn[client].hostname, conn[client].port, (char *) conn[client].initmessage,
                                    sizeof(conn[client].initmessage))) <= 0) {
...
void setrrcount(int i, long scount) {
...
void ssendbegining(int client, unsigned char *message, int size) /*send at more reliable beggining of packet */
{
    if (conn[client].opos + size + 6 > BUFFERSIZE)
        ssendbuffer(client);
...
void Quit(char *text) {
    ...
    for (n = 0; n < 5; n++) { /*repeat quiting messages to make sure that
...
static int contact() {
    ...
    GetSocketError(sock);
    if (DgramSend(Socket, DgramLastaddr(), DgramLastport(), (char *) connp->initmessage, sizeof(connp->initmessage)) ==
        -1) {
...
void wposs() {
    ...
    for (i = 0; i < connected; i++) {
        conn[client].sent = 0;
        if (/*conn[client].wpos && */ (conn[client].dispframes > conn[client].sentframes && conn[client].ready) ||
            (conn[client].failed && conn[client].sent)) {
            conn[client].counter += conn[client].dispframes;
            if ((conn[client].counter > conn[client].allframes) || (conn[client].failed && conn[client].sent)) {
                conn[client].sent = 1;
