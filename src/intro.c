 ...
 *
 */
void draw_koules(CONST int c, CONST int s, CONST int r) {
    int i;
    for (i = 0; i < 360; i += 60)
        switch (c) {
            case 0:
                PutBitmap((int) (MAPWIDTH / 2 - BALL_RADIUS / DIV + sin(RAD(i + s)) * r),
                          (int) (MAPHEIGHT / 2 - BALL_RADIUS / DIV + cos(RAD(i + s)) * r), ball_bitmap, physicalscreen);
                break;
            case 1:
                PutBitmap((int) (MAPWIDTH / 2 - BALL_RADIUS / DIV + sin(RAD(i + s)) * r),
                          (int) (MAPHEIGHT / 2 - BALL_RADIUS / DIV + cos(RAD(i + s)) * r), lball_bitmap[0],
                          physicalscreen);
                break;
            default:
                PutBitmap((int) (MAPWIDTH / 2 - BALL_RADIUS / DIV + sin(RAD(i + s)) * r),
                          (int) (MAPHEIGHT / 2 - BALL_RADIUS / DIV + cos(RAD(i + s)) * r), lball_bitmap[1],
                          physicalscreen);
                break;
        }
}
...
void koulescreator(CONST int r) {
    ...
    for (i = 0; i < 360; i += 60) {
        ...
        for (z1 = 0; z1 < BALL_RADIUS * BALL_RADIUS * M_PI / DIV / DIV; z1++) {
            x = rand() % GAMEWIDTH;
            y = rand() % GAMEHEIGHT;
            addpoint(x * 256 * DIV, y * 256 * DIV, (x1 - x) * 256 / (time) *DIV, (y1 - y) * 256 / (time) *DIV,
                     ball(rand() % 32), time);
        }
    }
}
void starcreator() {
    ...
    for (z = 0; z < ROCKET_RADIUS * ROCKET_RADIUS * M_PI / DIV / DIV; z++) {
        x = rand() % GAMEWIDTH;
        y = rand() % GAMEHEIGHT;
        addpoint(x * 256 * DIV, y * 256 * DIV, (MAPWIDTH / 2 - x) * 256 / (time) *DIV,
                 (MAPHEIGHT / 2 - y) * 256 / (time) *DIV, (rand() % 32), time);
    }
}
...
void draw_player(CONST int x, CONST int y, CONST float r, CONST int z, CONST int r1) {
    int x1, y1;
    PutBitmap((int) x - ROCKET_RADIUS / DIV, (int) y - ROCKET_RADIUS / DIV, rocket_bitmap[z], physicalscreen);
    x1 = x * DIV + sin(r - RAD(30)) * r1 - EYE_RADIUS;
    y1 = y * DIV + cos(r - RAD(30)) * r1 - EYE_RADIUS;
    PutBitmap(x1 / DIV, y1 / DIV, eye_bitmap[z], physicalscreen);
    x1 = x * DIV + sin(r + RAD(30)) * r1 - EYE_RADIUS;
    y1 = y * DIV + cos(r + RAD(30)) * r1 - EYE_RADIUS;
    PutBitmap(x1 / DIV, y1 / DIV, eye_bitmap[z], physicalscreen);
}
...
void starwars() {
    ...
    r[1] = r[2] = r[0] = sqrt((MAPWIDTH / 2) * (MAPWIDTH / 2) + (MAPHEIGHT / 2) * (MAPHEIGHT / 2));
    rp[0] = 0.0 / DIV;
    rp[1] = 0.6 / DIV;
    rp[2] = 0.6 / DIV;
    r1 = r[0];
    ...
    for (i = -660; i < (TEXTSIZE + 10) * TEXTW; i += 1) {
        ...
        if (wait >= 0 || tbreak) {
            ...
            if (bbally > -20 && bballx)
                ...
                PutBitmap((int) bballx - (BBALL_RADIUS) / DIV, bbally, bball_bitmap, physicalscreen);
        ...
        if (actu >= BLINE && !bballx)
            bballx = MAPWIDTH / 2, bbally = MAPHEIGHT + 30;
        if (bballx)
            bbally--;
...
void staraccel(CONST float x1, CONST float y1, CONST float r) {
    int y;
    for (y = 0; y < 5 / DIV / DIV; y++) {
        float p;
        p = RAD(rand() % 45 - 22);
        addpoint(x1 * 256, y1 * 256, (-sin(r + p) * 0.08 * 10) * (rand() % 512),
                 (-cos(r + p) * 0.08 * 10) * (rand() % 512), rocket(rand() % 16), 30);
    }
}
void outro2() {
    int y, i, z;
