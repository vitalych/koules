...
/* The samples to load */
char *FILENAME[] = {"start.raw",    "end.raw",      "colize.raw",  "destroy1.raw",
                    "destroy2.raw", "creator1.raw", "creator2.raw"};
...

    {
...

        {
            ...
            for (i = 0; i < fragsize; i++) {
                ...
                final[i + finalBuf * fragsize] =
                    (premix[i] > 255) ? 255 : (premix[i] < -256 ? 0 : (premix[i] >> 1) + 128);
        ...
        mciRC = mciSendCommand(usSoundDeviceID, MCI_STOP, MCI_WAIT, (PVOID) &mciOpenParameters, 0);
        ...
        /* rewind sound */
        mciRC = mciSendCommand(usSoundDeviceID, MCI_SEEK, MCI_TO_START, (PVOID) &mciOpenParameters, 0);

        /* play sound */
        mciRC = mciSendCommand(usSoundDeviceID, MCI_PLAY, MCI_WAIT, (PVOID) &mciOpenParameters, 0);
...
void init_sound(void) {
    ...
    mciRC = mciSendCommand(0, MCI_OPEN, ulOpenFlags, (PVOID) &mciOpenParameters, 0);
    ...
    mciRC = mciSendCommand(usSoundDeviceID, MCI_SET,
                           (MCI_WAIT | MCI_WAVE_SET_SAMPLESPERSEC | MCI_WAVE_SET_BITSPERSAMPLE | MCI_WAVE_SET_CHANNELS),
                           (PVOID) &mwspWaveFormParameters, 0);
...
void close_device(void) {
    ...
    /* close devide */
    mciSendCommand(mciOpenParameters.usDeviceID, MCI_CLOSE, MCI_WAIT, (PVOID) &mciGenericParms, (ULONG) NULL);
