...
void setmonopalette() {
    ...
    notusedc = 3;
    for (i = 0; i < 256; i++)
        pixels[i] = BlackPixel(dp, screen), spixels[i] = 0;
    for (i = 0; i < 16; i++) {
    ...
    pixels[0] = BlackPixel(dp, screen), pixels[255] = WhitePixel(dp, screen), spixels[0] = 0;
}
static int setcustompalette1(CONST int ncolors) {
...
void setcustompalette(CONST int p, CONST float p1) {
    ...
    colormap = XCreateColormap(dp, wi, DefaultVisual(dp, XDefaultScreen(dp)), AllocAll);
...
void Fade(CONST int steps) {
    ...
    if (useprivate) {
        ...
        XQueryColors(dp, colormap, orig_colors, NUM_COLORS);
        if (state == XFADE_OUT) {
            new_cmap = XCreateColormap(dp, DefaultRootWindow(dp), DefaultVisual(dp, XDefaultScreen(dp)), AllocAll);
    ...
    if (state == XFADE_OUT) {
        for (i = steps; i > 0; i--) {
            for (j = 0; j < NUM_COLORS; j++) {
                curr_colors[j].red = orig_colors[j].red * i / steps;
                curr_colors[j].green = orig_colors[j].green * i / steps;
                curr_colors[j].blue = orig_colors[j].blue * i / steps;
            }
            XStoreColors(dp, *fade_cmap, curr_colors, NUM_COLORS);
            usleep(10);
            XSync(dp, False);
        }
    } else {
    ...
    if (state == XFADE_IN) {
        ...
        if (useprivate) {
            XStoreColors(dp, colormap, orig_colors, NUM_COLORS);
        } else {
