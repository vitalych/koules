...
//    }
//}
//#define gl_putpixel(x,y,c) FillRectangle(x,y,y>3 * MAPHEIGHT / 4 && DIV == 1?2:1,y>3 * MAPHEIGHT / 5 && DIV ==
//1?2:1,c)
//
//
//
...
// DKS #define gl_putpixel(x,y,c) FillRectangle(x,y,y>3 * MAPHEIGHT / 4 && DIV == 1?2:1,y>3 * MAPHEIGHT / 5 && DIV ==
// 1?2:1,c)
...
#define vga_drawline(x1, y1, x2, y2)                                                       \
    SDL_DrawLine(physicalscreen, ((int) (MAPWIDTH / 2 + (x1) *220 / (1000 - (y1)) / DIV)), \
                 ((int) (MAPHEIGHT / 3 + MAPWIDTH * 220 / (1000 - (y1)))),                 \
                 ((int) (MAPWIDTH / 2 + (x2) *220 / (1000 - (y2)) / DIV)),                 \
                 ((int) (MAPHEIGHT / 3 + MAPWIDTH * 220 / (1000 - (y2)))), colors_RGBA[textcolor])
...
//            colors_RGBA[textcolor])
#define vga_drawhline(x1, y1, x2, y2)                                                     \
    SDL_DrawHLine(physicalscreen, (int) (MAPWIDTH / 2 + (x1) *220 / (1000 - (y1)) / DIV), \
                  (int) (MAPHEIGHT / 3 + MAPWIDTH * 220 / (1000 - (y1))),                 \
                  (int) (MAPWIDTH / 2 + (x2) *220 / (1000 - (y2)) / DIV), colors_RGBA[textcolor])
...
//
//
//  SSetPixel(physicalscreen, ((int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV)),
//  ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1)))), textcolor);
//
//  if SDL_MUSTLOCK(physicalscreen)
//    SDL_UnlockSurface(physicalscreen);
//}
...
#define BEST static
#endif
BEST void fontc_ul(x, y, r) CONST int x, y, r;
{
    int r34;
...
BEST void fontc_ur(x, y, r) CONST int x, y, r;
{
    int r34;
...
BEST void fontc_ll(x, y, r) CONST int x, y, r;
{
    int r34;
...
BEST void fontc_lr(x, y, r) CONST int x, y, r;
{
    int r34;
...
static void fontc_l(x, y, r) CONST int x, y, r;
{
    fontc_ll(x, y, r);
    fontc_lr(x, y, r);
}

static void fontc_u(x, y, r) CONST int x, y, r;
{
    fontc_ul(x, y, r);
    fontc_ur(x, y, r);
}

static void fontc_r(x, y, r) CONST int x, y, r;
{
    fontc_ur(x, y, r);
    fontc_lr(x, y, r);
}

static void fontc_left(x, y, r) CONST int x, y, r;
{
    fontc_ul(x, y, r);
    fontc_ll(x, y, r);
}

static void fontcircle(x, y, r) CONST int x, y, r;
{
    fontc_u(x, y, r);
    fontc_l(x, y, r);
}

static INLINE char *myindex(char *s, CONST int c) {
    for (; *s != 0; s++)
        if (*s == c)
            return (s);
    return (NULL);
...
int vgadrawtext(x, y, siz, str) CONST int x, y, siz;
CONST char *str;
{
    ...
    for (f = 0; f < strlen(str); f++) {
        ...
        if (!go_through_the_motions) {           /* only draw it if we really want to */
            ...
            else { /* common part-circle positions */
...
int vgatextsize(sizearg, str) CONST int sizearg;
CONST char *str;
{
    int r;
...
void set_max_text_width(width) CONST int width;
