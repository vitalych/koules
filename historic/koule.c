...
#define NSAMPLES 5
char *files[NSAMPLES] = {"start.raw", "end.raw", "colize.raw", "destroy1.raw", "destroy2.raw"};
...
static void create_bitmap() {
    ...
    for (x = 0; x < BALL_RADIUS * 2; x++)
        for (y = 0; y < BALL_RADIUS * 2; y++) {
            if ((x - BALL_RADIUS) * (x - BALL_RADIUS) + (y - BALL_RADIUS) * (y - BALL_RADIUS) <
                ...
                r = (x - BALL_XCENTER) * (x - BALL_XCENTER) + (y - BALL_XCENTER) * (y - BALL_XCENTER);
    ...
    if ((ball_bitmap = malloc(size)) == NULL)
        perror("balls"), exit(1);
    ...
    for (x = 0; x < ROCKET_RADIUS * 2; x++)
        for (y = 0; y < ROCKET_RADIUS * 2; y++) {
            if ((x - ROCKET_RADIUS) * (x - ROCKET_RADIUS) + (y - ROCKET_RADIUS) * (y - ROCKET_RADIUS) <
                ...
                r = (x - ROCKET_XCENTER) * (x - ROCKET_XCENTER) + (y - ROCKET_XCENTER) * (y - ROCKET_XCENTER);
                r = r * NCOLORS / ROCKET_SIZE_MAX;
    ...
    if ((rocket_bitmap = malloc(size)) == NULL)
        perror("rocket"), exit(1);
    ...
    for (po = 0; po < MAXROCKETS; po++) {
        for (x = 0; x < EYE_RADIUS * 2; x++)
            for (y = 0; y < EYE_RADIUS * 2; y++) {
                if ((x - EYE_RADIUS) * (x - EYE_RADIUS) + (y - EYE_RADIUS) * (y - EYE_RADIUS) <
                    ...
                    r = (x - EYE_XCENTER) * (x - EYE_XCENTER) + (y - EYE_XCENTER) * (y - EYE_XCENTER);
        ...
        if ((eye_bitmap[po] = malloc(size)) == NULL)
            perror("eye"), exit(1);
...
static void move_objects() {
    ...
    for (i = 0; i < nobjects; i++)
        if (object[i].type == CREATOR) {
            object[i].time--;
            if (!object[i].time) {
                object[i].live = 1, object[i].type = BALL;
            }
...
static void points() {
    ...
    for (i = 0; i < MAXPOINT; i++) {
        if (point[i].time > 0) {
            ...
            if (point[i].x > 0 && point[i].x >> 8 < MAPWIDTH && point[i].y > 0 && point[i].y >> 8 < MAPHEIGHT)
    ...
    for (i = 0; i < nobjects; i++)
        if (object[i].live) {
            if (object[i].type == BALL || object[i].type == LBALL) {
                gl_putboxmaskcompiled((int) object[i].x - BALL_RADIUS, (int) object[i].y - BALL_RADIUS, BALL_RADIUS * 2,
                                      BALL_RADIUS * 2, ball_bitmap);
            ...
            if (object[i].type == ROCKET) {
                ...
                gl_putboxmaskcompiled(x1, y1, EYE_RADIUS * 2, EYE_RADIUS * 2, eye_bitmap[i]);
                x1 = object[i].x + sin(object[i].rotation + RAD(30)) * EYE_RADIUS1 - EYE_RADIUS;
                y1 = object[i].y + cos(object[i].rotation + RAD(30)) * EYE_RADIUS1 - EYE_RADIUS;
                gl_putboxmaskcompiled(x1, y1, EYE_RADIUS * 2, EYE_RADIUS * 2, eye_bitmap[i]);
            }
    ...
    for (i = 0; i < RAD(360); i += RAD(2)) {
        ...
        switch (type) {
            case BALL:
                ...
                break;
            case ROCKET:
                color = rocket(rand() % 32);
                break;
        }
        addpoint(x * 256, y * 256, sin(i) * (speed), cos(i) * (speed), color, rand() % 1000);
    }
}
            ...
            if (object[i].x - object[i].radius < 0 || object[i].x + object[i].radius > MAPWIDTH ||
                ...
                switch (object[i].type) {
                    case LBALL:
                        ...
                        object[i].live = 0, explosion(object[i].x, object[i].y, object[i].type);
                        break;
                    case BALL:
                        Snd_effect(S_DESTROY_BALL, next);
                        if (rand() % 2 == 0) {
...
static void process_keys() {
    ...
    if (keyboard_keypressed(SCANCODE_CURSORUP)) {
        ...
        for (i = 0; i < 5; i++) {
            p = RAD(rand() % 45 - 22);
            addpoint(object[0].x * 256, object[0].y * 256,
    ...
    if (keyboard_keypressed(SCANCODE_CURSORBLOCKUP)) {
        ...
        for (i = 0; i < 5; i++) {
            p = RAD(rand() % 45 - 22);
            addpoint(object[1].x * 256, object[1].y * 256,
...
static void creator() {
    ...
    int z;
    int x, y;
    for (i = nobjects; i < MAXOBJECT && (object[i].live || object[i].type == CREATOR); i++)
        ;
    if (i == MAXOBJECT)
        return;
    if (i >= nobjects)
    ...
    for (z = 0; z < 360; z++) {
        x = rand() % MAPWIDTH;
        y = rand() % MAPHEIGHT;
        addpoint(x * 256, y * 256, (object[i].x - x) * 256 / (time), (object[i].y - y) * 256 / (time),
                 ball(rand() % 32), time);
    }
}
