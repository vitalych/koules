...
#if 0
#if DIV == 1
#define BSetPixel(bitmap, x, y, color) \
    *(BitmapToBuffer(bitmap) + (x) + (((y) &0xffffff00) >> 1) + (((y) &0xffffff00) << 1)) = color
#define SMySetPixel(screen, x, y, color) \
    *(VScreenToBuffer(screen) + (x) + (((y) &0xffffff00) >> 1) + (((y) &0xffffff00) << 1)) = color
/*#define SSetPixel(x,y,color) *((currentcontext.vbuf)+(x)+(y)*MAPWIDTH) )=color */
#else
#define BSetPixel(bitmap, x, y, color) \
    *(BitmapToBuffer(bitmap) + (x) + ((y) &0xffffff00) + (((y) &0xffffff00) >> 2)) = color
/*#define SSetPixel(x,y,color) *((currentcontext.vbuf)+(x)+((y)*MAPWIDTH))=color */
#define SMySetPixel(screen, x, y, color) \
    *(VScreenToBuffer(screen) + (x) + ((y) &0xffffff00) + (((y) &0xffffff00) >> 2)) = color
...
extern inline void DrawBlackMaskedText(int x, int y, char *text) {
    gl_setwritemode(WRITEMODE_MASKED);
    gl_setfont(8, 8, fontblack);
    gl_write(x, y, text);
}
extern inline void DrawWhiteMaskedText(int x, int y, char *text) {
    gl_setwritemode(WRITEMODE_MASKED);
    gl_setfont(8, 8, fontwhite);
    gl_write(x, y, text);
}
extern inline void DrawText(int x, int y, char *text) {
...
extern inline char *CompileBitmap(int x, int y, char *bitmap) {
    int size1;
    char *cbitmap;
...
extern inline void CopyVSToVS(VScreenType source, VScreenType destination) {
    SetScreen(source);
...
extern inline void CopyToScreen(VScreenType source) {
    SetScreen(source);
    if (!flipping)
