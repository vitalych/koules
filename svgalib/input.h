...
extern inline int Pressed() {
    ...
    if (joystickdevice[0] >= 0) {
        ...
        if ((status == JS_RETURN) && js.buttons)
            return 1;
    }
#endif
#ifdef MOUSE
...
#define IsPressedEnter() (IsPressed(SCANCODE_ENTER) || IsPressed(SCANCODE_SPACE) || IsPressed(SCANCODE_KEYPADENTER))

#define IsPressedUp() (IsPressed(SCANCODE_CURSORUP) || IsPressed(SCANCODE_CURSORBLOCKUP))

#define IsPressedDown() (IsPressed(SCANCODE_CURSORDOWN) || IsPressed(SCANCODE_CURSORBLOCKDOWN))
#define IsPressedLeft() (IsPressed(SCANCODE_CURSORLEFT) || IsPressed(SCANCODE_CURSORBLOCKLEFT))

#define IsPressedRight() (IsPressed(SCANCODE_CURSORRIGHT) || IsPressed(SCANCODE_CURSORBLOCKRIGHT))
