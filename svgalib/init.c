...
static void initialize() {
    ...
    gl_setclippingwindow(0, 0, MAPWIDTH - 1, MAPHEIGHT + 19);
    gl_getcontext(physicalscreen);
    if (flipping) {
        if (physicalscreen->modetype != CONTEXT_LINEAR || !gl_enablepageflipping(physicalscreen))
            flipping = 0;
        else {
...
static void uninitializes(int num) {
    char s[256];
    sprintf(s, "Signal %i!!!\n", num);
    CQuit(s);
    uninitialize();
